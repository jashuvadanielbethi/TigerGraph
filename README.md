...
## Deploy to Vercel (any transaction ID, real investigation)

`public/` is the website and `api/investigate.py` is a Vercel serverless
function that runs the **unchanged pipeline** on any transaction ID from the
dataset. `data/deploy/*.pkl.gz` (about 28 MB) is the compressed lean dataset
built from `transactions.csv`, `identity.csv` and `closed_cases_history.csv`
(the raw 700 MB CSV cannot be stored in GitHub, which limits files to 100 MB).
The function unpacks it on first use, so the first request after a period of
inactivity can take a few seconds.

1. Import the GitHub repo in Vercel with all defaults (no framework, build
   command, or environment variables). `vercel.json` already configures the
   function and the static site; `requirements.txt` is the slim set of
   dependencies the function needs.
2. Deploy, open the site, and enter any transaction ID.

The 20 files in `cases/` are only the graded showcase and are not used for
lookups.

To rebuild `data/deploy/` after changing the dataset, run
`python data_ingest/build_index.py` and then gzip the three files in
`data/processed/` into `data/deploy/` (`transactions`, `identity`,
`closed_cases`, each as `<name>.pkl.gz`).

Locally, `python api_server.py` (http://localhost:8000) does the same job and
the page uses it automatically when opened from `localhost`.
...
| `api/investigate.py` | Vercel function: real investigation for any transaction ID |
| `public/` | Website (New investigation, Case detail, Overview) |
