...
**How New investigation finds a transaction** (the 20 files in `cases/` are
only the graded showcase and are *not* used for lookups):

1. If the local backend is running, it calls it and shows the full result
   (evidence, similar cases, SAR narrative). Start it with
   `python api_server.py` (http://localhost:8000; use `?api=http://host:port`
   on the page URL for another host).
2. Otherwise it reads pre-computed results from `public/lookup/<trigger>/`,
   generated by running the unchanged pipeline over every transaction in
   `transactions.csv`. These show the verdict, probability, pattern,
   exposure, connected cards, and next best action before/after evidence
   (without the full evidence table).

Generate or refresh the saved results (resumable; about 2.5 hours per trigger
on 4 workers, needs about 400 MB of RAM per worker):

```bash
python tools/precompute_lookup.py --trigger risk_score --workers 4
python tools/precompute_lookup.py --trigger customer_report --workers 4
python tools/precompute_lookup.py --trigger analyst_request --workers 4
```

Transactions whose shard has not been generated yet show a "still being
prepared" message. Check `public/lookup/<trigger>/` to see which shards exist
(one file per 1,000 transaction IDs, 591 per trigger).
